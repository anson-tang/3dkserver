...
def call_db(p):
    if p:
        res = yield p.call('randcard', [2, 1])
        msg( 'res: {0}.'.format(res) )
        if not res[0] and res[1][1]:
            ufid, _ = res[1][1][0]
            res = yield p.call('decompose', [1, [ufid]])
            msg( 'res: {0}.'.format(res) )
 
    reactor.callLater(INTERVAL_RANDCARD, call_db, p)



def connectServer():
    for _i in range(0, TOTAL_USER):
        conn = ConnectorCreator( call_randcard )
        d = conn.connect(HOST, PORT)
 
        account = u'test%s' % (_i+1)
        args = [u'1', account, u'sessionkey', u'1']
        d.addCallbacks( call_randcard, errback, (args, ) )
