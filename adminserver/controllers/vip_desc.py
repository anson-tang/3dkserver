...
BACK_FILE = 'vip_desc_back_up.xls'

field_vip_desc = 'ID', 'VipLevel', 'VipDesc', 'Sign'
sql_vip_desc   = 'SELECT %s FROM tb_vip_desc' % ','.join(field_vip_desc)
insert_sql_skill   = 'INSERT INTO tb_vip_desc (%s) VALUES (%s)' % (','.join(field_vip_desc), ','.join(['%s'] * len(field_vip_desc)))

IMPORT_SQLS = {
        'VipDesc'   : [insert_sql_skill, 'tb_vip_desc'],
    }

EXPORT_SQLS = {
        'tb_vip_desc' : [sql_vip_desc, field_vip_desc, 'VipDesc'],
    }

@route("/vip_desc")
def vip_desc_index(db):
    db.execute(sql_vip_desc)
    data = db.fetchall()
    return template('vip_desc_index', data=data)

@route("/vip_desc/delete", method="POST")
def vip_desc_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_vip_desc WHERE ID=%s;' % sid
...
@route("/vip_desc/import", method="POST")
def vip_desc_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_vip_desc%Y%m%d%H%M%S.xls'))
...
@route("/vip_desc/export")
def vip_desc_export(db):
    tmp_root = './tmp/'
    filename = current_time("vip_desc_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/vip_desc/recover")
def vip_desc_recover(db):
    tmp_root = './tmp/'
