...
MONSTER_BACK_FILE = 'monsterskill_back_up.xls'

field_monster_skill = 'ID', 'BeMove', 'IsTargetBlock', 'IsTargetMiss', 'IsTargetCounter', 'HitArea', 'BonusDamage', 'Target', 'Sound', 'BeHitTime', 'Buffid', 'FloadWord'
sql_monster_skill  = 'SELECT %s FROM tb_monster_skill' % ','.join(field_monster_skill)
insert_sql_skill   = 'INSERT INTO tb_monster_skill (%s) VALUES (%s)' % (','.join(field_monster_skill), ','.join(['%s'] * len(field_monster_skill)))

MONSTER_IMPORT_SQLS = {
        'Skill'   : [insert_sql_skill, 'tb_monster_skill'],
    }

MONSTER_EXPORT_SQLS = {
        'tb_monster_skill' : [sql_monster_skill, field_monster_skill, 'Skill'],
    }

@route("/monsterskill")
def monster_skill_index(db):
    db.execute(sql_monster_skill)
    data = db.fetchall()
    return template('monster_skill', data=data)

@route("/monsterskill/delete", method="POST")
def monster_skill_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_monster_skill WHERE ID=%s;' % sid
...
@route("/monsterskill/import", method="POST")
def monster_skill_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmpmonsters_%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/monsterskill")

@route("/monsterskill/export")
def monster_skill_export(db):
    tmp_root = './tmp/'
    filename = current_time("monsters_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/monsterskill/recover")
def monster_skill_recover(db):
    tmp_root = './tmp/'
