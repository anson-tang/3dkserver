...
BACK_FILE = 'randcard_item_back_up.xls'
table_name = 'tb_randcard_level'

field_base = 'Level', 'GreenCount', 'BlueCount', 'PurpleCount'
...
IMPORT_SQLS = {
        'RandCardLevel' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'RandCardLevel'],
    }

@route("/randcard_level")
def randcard_level_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('randcard_level_index', data=data)

@route("/randcard_level/delete", method="POST")
def randcard_level_delete(db):
...
@route("/randcard_level/import", method="POST")
def randcard_level_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmprandcard_level_%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/randcard_level")

@route("/randcard_level/export")
def randcard_level_export(db):
    tmp_root = './tmp/'
    filename = current_time("randcard_level_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/randcard_level/recover")
def randcard_level_recover(db):
    tmp_root = './tmp/'
