...
BACK_FILE = 'monthly_card_back_up.xls'

field_monthly_card = 'ID', 'ItemType', 'ItemID', 'ItemNum'
del_sql_monthly_card = 'DELETE FROM tb_monthly_card WHERE ID=%s;'

sql_monthly_card   = 'SELECT %s FROM tb_monthly_card' % ','.join(field_monthly_card)
insert_sql_monthly_card   = 'INSERT INTO tb_monthly_card (%s) VALUES (%s)' % (','.join(field_monthly_card), ','.join(['%s'] * len(field_monthly_card)))

excel_name = 'MonthlyCard'
excel_export_filename = "MonthlyCard_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_monthly_card, 'tb_monthly_card'],
    }

EXPORT_SQLS = {
        'tb_monthly_card' : [sql_monthly_card, field_monthly_card, excel_name],
    }

@route("/monthly_card")
def monthly_card_index(db):
    db.execute(sql_monthly_card)
    data = db.fetchall()
    return template('monthly_card_index', data=data)

@route("/monthly_card/delete", method="POST")
def monthly_card_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_monthly_card WHERE ID=%s;' % sid
    sql = del_sql_monthly_card % sid
...
@route("/monthly_card/import", method="POST")
def monthly_card_import(db):
    ...
    if error:
        ...
        redirect("/monthly_card")

@route("/monthly_card/export")
...
@route("/monthly_card/recover")
def monthly_card_recover(db):
    tmp_root = './tmp/'
