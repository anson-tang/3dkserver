...
BACK_FILE = 'refine_back_up.xls'

field_refine = 'ItemID', 'StrengThenLV', 'BasicRefineMin', 'BasicRefineMax', 'MiddleRefineMin', 'MiddleRefineMax', 'TopRefineMin', 'TopRefineMax', 'AttributeID', 'AttributeMax' 
sql_refine   = 'SELECT %s FROM tb_refine' % ','.join(field_refine)
insert_sql_skill   = 'INSERT INTO tb_refine (%s) VALUES (%s)' % (','.join(field_refine), ','.join(['%s'] * len(field_refine)))

IMPORT_SQLS = {
        'Refine'   : [insert_sql_skill, 'tb_refine'],
    }

EXPORT_SQLS = {
        'tb_refine' : [sql_refine, field_refine, 'Refine'],
    }

@route("/refine")
def refine_index(db):
    db.execute(sql_refine)
    data = db.fetchall()
    return template('refine_index', data=data)

@route("/refine/delete", method="POST")
def refine_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_refine WHERE ID=%s;' % sid
...
@route("/refine/import", method="POST")
def refine_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_refine%Y%m%d%H%M%S.xls'))
...
@route("/refine/export")
def refine_export(db):
    tmp_root = './tmp/'
    filename = current_time("refine_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/refine/recover")
def refine_recover(db):
    tmp_root = './tmp/'
