...
BACK_FILE = 'cardpool_back_up.xls'
table_name = 'tb_cardpool'

field_base = 'ID', 'CardID', 'Level', 'StartLevel', 'Rate', 'ItemType', 'ItemId', 'ItemNum', 'Notice'
...
IMPORT_SQLS = {
        'CardPool' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'CardPool'],
    }

@route("/cardpool")
def cardpool_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('cardpool_index', data=data)

@route("/cardpool/delete", method="POST")
def cardpool_delete(db):
...
@route("/cardpool/import", method="POST")
def cardpool_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmpcardpool_%Y%m%d%H%M%S.xls'))
...
@route("/cardpool/export")
def cardpool_export(db):
    tmp_root = './tmp/'
    filename = current_time("cardpool_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/cardpool/recover")
def cardpool_recover(db):
    tmp_root = './tmp/'
