...
BACK_FILE = 'game_sound_back_up.xls'

field_game_sound = 'SystemKey', 'SoundName'
sql_game_sound   = 'SELECT %s FROM tb_game_sound' % ','.join(field_game_sound)
insert_sql_skill   = 'INSERT INTO tb_game_sound (%s) VALUES (%s)' % (','.join(field_game_sound), ','.join(['%s'] * len(field_game_sound)))

IMPORT_SQLS = {
        'GameSound'   : [insert_sql_skill, 'tb_game_sound'],
    }

EXPORT_SQLS = {
        'tb_game_sound' : [sql_game_sound, field_game_sound, 'GameSound'],
    }

@route("/game_sound")
def game_sound_index(db):
    db.execute(sql_game_sound)
    data = db.fetchall()
    return template('game_sound_index', data=data)

@route("/game_sound/delete", method="POST")
def game_sound_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_game_sound WHERE ID=%s;' % sid
...
@route("/game_sound/import", method="POST")
def game_sound_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_game_sound%Y%m%d%H%M%S.xls'))
...
@route("/game_sound/export")
def game_sound_export(db):
    tmp_root = './tmp/'
    filename = current_time("game_sound_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/game_sound/recover")
def game_sound_recover(db):
    tmp_root = './tmp/'
