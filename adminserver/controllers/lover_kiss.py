...
BACK_FILE = 'lover_kiss_back_up.xls'

field_lover_kiss = 'KissNum', 'LuxuryRewardRate'
sql_lover_kiss   = 'SELECT %s FROM tb_lover_kiss' % ','.join(field_lover_kiss)
insert_sql_skill   = 'INSERT INTO tb_lover_kiss (%s) VALUES (%s)' % (','.join(field_lover_kiss), ','.join(['%s'] * len(field_lover_kiss)))

IMPORT_SQLS = {
        'Lover_kiss'   : [insert_sql_skill, 'tb_lover_kiss'],
    }

EXPORT_SQLS = {
        'tb_lover_kiss' : [sql_lover_kiss, field_lover_kiss, 'Lover_kiss'],
    }

@route("/lover_kiss")
def lover_kiss_index(db):
    db.execute(sql_lover_kiss)
    data = db.fetchall()
    return template('lover_kiss_index', data=data)

@route("/lover_kiss/delete", method="POST")
def lover_kiss_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_lover_kiss WHERE KissNum=%s;' % sid
...
@route("/lover_kiss/import", method="POST")
def lover_kiss_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_lover_kiss%Y%m%d%H%M%S.xls'))
...
@route("/lover_kiss/recover")
def lover_kiss_recover(db):
    tmp_root = './tmp/'
