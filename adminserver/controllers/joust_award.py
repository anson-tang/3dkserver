...
BACK_FILE = 'joust_award_back_up.xls'

field_joust_award = 'StartRank', 'EndRank', 'AwardList'
del_sql_joust_award = 'DELETE FROM tb_joust_award WHERE StartRank=%s;'

sql_joust_award   = 'SELECT %s FROM tb_joust_award' % ','.join(field_joust_award)
insert_sql_joust_award   = 'INSERT INTO tb_joust_award (%s) VALUES (%s)' % (','.join(field_joust_award), ','.join(['%s'] * len(field_joust_award)))

excel_name = 'JoustAward'
excel_export_filename = "JoustAward_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_joust_award, 'tb_joust_award'],
    }

EXPORT_SQLS = {
        'tb_joust_award' : [sql_joust_award, field_joust_award, excel_name],
    }

@route("/joust_award")
def joust_award_index(db):
    db.execute(sql_joust_award)
    data = db.fetchall()
    return template('joust_award_index', data=data)

@route("/joust_award/delete", method="POST")
def joust_award_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_joust_award WHERE ID=%s;' % sid
    sql = del_sql_joust_award % sid
...
@route("/joust_award/import", method="POST")
def joust_award_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_joust_award%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/joust_award")

@route("/joust_award/export")
def joust_award_export(db):
    tmp_root = './tmp/'
...
@route("/joust_award/recover")
def joust_award_recover(db):
    tmp_root = './tmp/'
