...
BACK_FILE = 'expand_bag_back_up.xls'

field_expand_bag = 'ID', 'BagType', 'BagCount', 'Cost'
sql_expand_bag   = 'SELECT %s FROM tb_expand_bag' % ','.join(field_expand_bag)
insert_sql_skill   = 'INSERT INTO tb_expand_bag (%s) VALUES (%s)' % (','.join(field_expand_bag), ','.join(['%s'] * len(field_expand_bag)))

IMPORT_SQLS = {
        'ExpandBag'   : [insert_sql_skill, 'tb_expand_bag'],
    }

EXPORT_SQLS = {
        'tb_expand_bag' : [sql_expand_bag, field_expand_bag, 'ExpandBag'],
    }

@route("/expand_bag")
def expand_bag_index(db):
    db.execute(sql_expand_bag)
    data = db.fetchall()
    return template('expand_bag', data=data)

@route("/expand_bag/delete", method="POST")
def expand_bag_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_expand_bag WHERE ID=%s;' % sid
...
@route("/expand_bag/import", method="POST")
def expand_bag_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_expand_bag%Y%m%d%H%M%S.xls'))
...
@route("/expand_bag/export")
def expand_bag_export(db):
    tmp_root = './tmp/'
    filename = current_time("expand_bag_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/expand_bag/recover")
def expand_bag_recover(db):
    tmp_root = './tmp/'
