...
BACK_FILE = 'chaos_back_up.xls'

field_chaos = 'ID', 'Name', 'AttributeID', 'AttributeValue', 'Color', 'CostStarNum', 'CostGold', 'ChangeRole'
del_sql_chaos = 'DELETE FROM tb_chaos WHERE ID=%s;'

sql_chaos   = 'SELECT %s FROM tb_chaos' % ','.join(field_chaos)
insert_sql_chaos   = 'INSERT INTO tb_chaos (%s) VALUES (%s)' % (','.join(field_chaos), ','.join(['%s'] * len(field_chaos)))

excel_name = 'Chaos'
excel_export_filename = "Chaos_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_chaos, 'tb_chaos'],
    }

EXPORT_SQLS = {
        'tb_chaos' : [sql_chaos, field_chaos, excel_name],
    }

@route("/chaos")
def chaos_index(db):
    db.execute(sql_chaos)
    data = db.fetchall()
    return template('chaos_index', data=data)

@route("/chaos/delete", method="POST")
def chaos_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_chaos WHERE ID=%s;' % sid
    sql = del_sql_chaos % sid
...
@route("/chaos/import", method="POST")
def chaos_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_chaos%Y%m%d%H%M%S.xls'))
