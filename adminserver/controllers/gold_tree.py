...
BACK_FILE = 'gold_tree_back_up.xls'

field_gold_tree = 'Damage', 'GoldReward'
del_sql_gold_tree = 'DELETE FROM tb_gold_tree WHERE Damage=%s;'

sql_gold_tree   = 'SELECT %s FROM tb_gold_tree' % ','.join(field_gold_tree)
insert_sql_gold_tree   = 'INSERT INTO tb_gold_tree (%s) VALUES (%s)' % (','.join(field_gold_tree), ','.join(['%s'] * len(field_gold_tree)))

excel_name = 'GoldTree'
excel_export_filename = "GoldTree_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_gold_tree, 'tb_gold_tree'],
    }

EXPORT_SQLS = {
        'tb_gold_tree' : [sql_gold_tree, field_gold_tree, excel_name],
    }

@route("/gold_tree")
def gold_tree_index(db):
    db.execute(sql_gold_tree)
    data = db.fetchall()
    return template('gold_tree_index', data=data)

@route("/gold_tree/delete", method="POST")
def gold_tree_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_gold_tree WHERE ID=%s;' % sid
    sql = del_sql_gold_tree % sid
...
@route("/gold_tree/import", method="POST")
def gold_tree_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_gold_tree%Y%m%d%H%M%S.xls'))
...
@route("/gold_tree/export")
def gold_tree_export(db):
    tmp_root = './tmp/'
...
@route("/gold_tree/recover")
def gold_tree_recover(db):
    tmp_root = './tmp/'
