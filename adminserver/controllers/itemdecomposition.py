...
BACK_FILE = 'itemdecomposition_back_up.xls'
table_name = 'tb_item_decomposition'

field_base = 'ItemType', 'QualityLevel', 'ItemList'
...
IMPORT_SQLS = {
        'ItemDecomposition' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'ItemDecomposition'],
    }

@route("/itemdecomposition")
def itemdecomposition_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('item_decomposition', data=data)

@route("/itemdecomposition/delete", method="POST")
def itemdecomposition_delete(db):
    itype = request.forms.itype.strip()
    quality = request.forms.quality.strip()
    sql = 'DELETE FROM %s WHERE ItemType=%s AND QualityLevel=%s' % (table_name, itype, quality)
...
@route("/itemdecomposition/import", method="POST")
def itemdecomposition_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmpitemdecomposition_%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/itemdecomposition")

@route("/itemdecomposition/export")
def itemdecomposition_export(db):
    tmp_root = './tmp/'
    filename = current_time("itemdecomposition_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/itemdecomposition/recover")
def itemdecomposition_recover(db):
    tmp_root = './tmp/'
