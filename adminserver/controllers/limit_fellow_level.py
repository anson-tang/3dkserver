...
BACK_FILE = 'limit_fellow_item_back_up.xls'
table_name = 'tb_limit_fellow_level'

field_base = 'Level', 'RandCount'
...
IMPORT_SQLS = {
        'LimitFellowLevel' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'LimitFellowLevel'],
    }

@route("/limit_fellow_level")
def limit_fellow_level_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('limit_fellow_level_index', data=data)

@route("/limit_fellow_level/delete", method="POST")
def limit_fellow_level_delete(db):
    level = request.forms.level.strip()
    sql = 'DELETE FROM %s WHERE Level=%s;' % (table_name, level)
...
@route("/limit_fellow_level/import", method="POST")
def limit_fellow_level_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmplimit_fellow_level_%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/limit_fellow_level")

@route("/limit_fellow_level/export")
def limit_fellow_level_export(db):
    tmp_root = './tmp/'
    filename = current_time("LimitFellowLevel_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/limit_fellow_level/recover")
def limit_fellow_level_recover(db):
    tmp_root = './tmp/'
