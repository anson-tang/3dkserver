...
BACK_FILE = 'arena_robot_back_up.xls'
TABLE_NAME = 'tb_arena_robot'

field_arena_robot = 'ID', 'RobotName', 'RobotList', 'RobotLevel'
sql_arena_robot   = 'SELECT %s FROM tb_arena_robot' % ','.join(field_arena_robot)
insert_sql_skill   = 'INSERT INTO tb_arena_robot (%s) VALUES (%s)' % (','.join(field_arena_robot), ','.join(['%s'] * len(field_arena_robot)))

excel_name = 'ArenaRobot'
excel_export_filename = "ArenaRobot_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_skill, 'tb_arena_robot'],
    }

EXPORT_SQLS = {
        'tb_arena_robot' : [sql_arena_robot, field_arena_robot, excel_name],
    }

@route("/arena_robot")
@route("/arena_robot/<lang:int>")
def arena_robot_index(db, lang=0):
    db.execute(sql_arena_robot)
    data = db.fetchall()
    return template('arena_robot_index', data=data, lang=lang)

@route("/arena_robot/page")
def arena_robot_page(db):
    ...
    return data
@route("/arena_robot/delete", method="POST")
def arena_robot_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_arena_robot WHERE ID=%s;' % sid
...
@route("/arena_robot/import", method="POST")
def arena_robot_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_arena_robot%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/arena_robot")

@route("/arena_robot/export")
def arena_robot_export(db):
    tmp_root = './tmp/'
...
@route("/arena_robot/recover")
def arena_robot_recover(db):
    tmp_root = './tmp/'
