...
BACK_FILE = 'growth_plan_back_up.xls'

field_growth_plan = 'PlanLevel', 'CreditsNum'
del_sql_growth_plan = 'DELETE FROM tb_growth_plan WHERE PlanLevel=%s;'

sql_growth_plan   = 'SELECT %s FROM tb_growth_plan' % ','.join(field_growth_plan)
insert_sql_growth_plan   = 'INSERT INTO tb_growth_plan (%s) VALUES (%s)' % (','.join(field_growth_plan), ','.join(['%s'] * len(field_growth_plan)))

excel_name = 'GrowthPlan'
excel_export_filename = "GrowthPlan_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_growth_plan, 'tb_growth_plan'],
    }

EXPORT_SQLS = {
        'tb_growth_plan' : [sql_growth_plan, field_growth_plan, excel_name],
    }

@route("/growth_plan")
def growth_plan_index(db):
    db.execute(sql_growth_plan)
    data = db.fetchall()
    return template('growth_plan_index', data=data)

@route("/growth_plan/delete", method="POST")
def growth_plan_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_growth_plan WHERE ID=%s;' % sid
    sql = del_sql_growth_plan % sid
...
@route("/growth_plan/import", method="POST")
def growth_plan_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_growth_plan%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/growth_plan")

@route("/growth_plan/export")
def growth_plan_export(db):
    tmp_root = './tmp/'
...
@route("/growth_plan/recover")
def growth_plan_recover(db):
    tmp_root = './tmp/'
