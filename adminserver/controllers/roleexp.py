...
BACK_FILE = 'roleexp_back_up.xls'
table_name = 'tb_roleexp'

field_base = 'Level', 'RoleExp', 'White', 'Green', 'Blue', 'Purple', 'Orange', 'Red'
...
IMPORT_SQLS = {
        'RoleExp' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'RoleExp'],
    }

@route("/roleexp")
def roleexp_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('roleexp_index', data=data)

@route("/roleexp/delete", method="POST")
def roleexp_delete(db):
...
@route("/roleexp/import", method="POST")
def roleexp_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmproleexp_%Y%m%d%H%M%S.xls'))
...
@route("/roleexp/export")
def roleexp_export(db):
    tmp_root = './tmp/'
    filename = current_time("roleexp_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/roleexp/recover")
def roleexp_recover(db):
    tmp_root = './tmp/'
