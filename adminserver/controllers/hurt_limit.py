...
BACK_FILE = 'hurt_limit_back_up.xls'

field_hurt_limit = 'RoleLevel', 'WorldBossHurt', 'GoldTreeHurt'
del_sql_hurt_limit = 'DELETE FROM tb_hurt_limit WHERE RoleLevel=%s;'

sql_hurt_limit   = 'SELECT %s FROM tb_hurt_limit' % ','.join(field_hurt_limit)
insert_sql_hurt_limit   = 'INSERT INTO tb_hurt_limit (%s) VALUES (%s)' % (','.join(field_hurt_limit), ','.join(['%s'] * len(field_hurt_limit)))

excel_name = 'HurtLimit'
excel_export_filename = "HurtLimit_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_hurt_limit, 'tb_hurt_limit'],
    }

EXPORT_SQLS = {
        'tb_hurt_limit' : [sql_hurt_limit, field_hurt_limit, excel_name],
    }

@route("/hurt_limit")
def hurt_limit_index(db):
    db.execute(sql_hurt_limit)
    data = db.fetchall()
    return template('hurt_limit_index', data=data)

@route("/hurt_limit/delete", method="POST")
def hurt_limit_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_hurt_limit WHERE ID=%s;' % sid
    sql = del_sql_hurt_limit % sid
...
@route("/hurt_limit/import", method="POST")
def hurt_limit_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_hurt_limit%Y%m%d%H%M%S.xls'))
...
@route("/hurt_limit/export")
def hurt_limit_export(db):
    tmp_root = './tmp/'
...
@route("/hurt_limit/recover")
def hurt_limit_recover(db):
    tmp_root = './tmp/'
