...
BACK_FILE = 'treasure_exp_back_up.xls'

field_treasure_exp = 'TreasureLevel', 'WhiteCost', 'GreenCost', 'BlueCost', 'PurpleCost', 'OrangeCost'
sql_treasure_exp   = 'SELECT %s FROM tb_treasure_exp' % ','.join(field_treasure_exp)
insert_sql_skill   = 'INSERT INTO tb_treasure_exp (%s) VALUES (%s)' % (','.join(field_treasure_exp), ','.join(['%s'] * len(field_treasure_exp)))

IMPORT_SQLS = {
        'TreasureExp'   : [insert_sql_skill, 'tb_treasure_exp'],
    }

EXPORT_SQLS = {
        'tb_treasure_exp' : [sql_treasure_exp, field_treasure_exp, 'TreasureExp'],
    }

@route("/treasure_exp")
def treasure_exp_index(db):
    db.execute(sql_treasure_exp)
    data = db.fetchall()
    return template('treasure_exp_index', data=data)

@route("/treasure_exp/delete", method="POST")
def treasure_exp_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_treasure_exp WHERE ID=%s;' % sid
...
@route("/treasure_exp/import", method="POST")
def treasure_exp_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_treasure_exp%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/treasure_exp")

@route("/treasure_exp/export")
def treasure_exp_export(db):
    tmp_root = './tmp/'
    filename = current_time("treasure_exp_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/treasure_exp/recover")
def treasure_exp_recover(db):
    tmp_root = './tmp/'
