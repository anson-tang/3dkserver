...
BACK_FILE = 'floadword_back_up.xls'
table_name = 'tb_floadword'

field_base = 'FlowdWordID', 'Bottom', 'Icon', 'Word'
...
IMPORT_SQLS = {
        'Sheet1' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'Sheet1'],
    }

@route("/floadword")
def floadword_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('floadword_index', data=data)

@route("/floadword/delete", method="POST")
def floadword_delete(db):
    fid = request.forms.fid.strip()
    sql = 'DELETE FROM %s WHERE FlowdWordID=%s' % (table_name, fid)
...
@route("/floadword/import", method="POST")
def floadword_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmpfloadword_%Y%m%d%H%M%S.xls'))
...
@route("/floadword/export")
def floadword_export(db):
    tmp_root = './tmp/'
    filename = current_time("floadword_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/floadword/recover")
def floadword_recover(db):
    tmp_root = './tmp/'
