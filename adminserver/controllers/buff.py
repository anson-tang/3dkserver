...
BACK_FILE = 'buff_back_up.xls'

field_buff = 'BuffID', 'BuffType', 'FloatWord', 'BuffEffect', 'FloatX', 'FloatY', 'FloatZ', 'Target', 'BuffArea', 'RoundCount', 'IsActionTakeEffect', 'Probability', 'BuffAttribute', 'BuffAttributeValue'
sql_buff   = 'SELECT %s FROM tb_buff' % ','.join(field_buff)
insert_sql_skill   = 'INSERT INTO tb_buff (%s) VALUES (%s)' % (','.join(field_buff), ','.join(['%s'] * len(field_buff)))

IMPORT_SQLS = {
        'Buff'   : [insert_sql_skill, 'tb_buff'],
    }

EXPORT_SQLS = {
        'tb_buff' : [sql_buff, field_buff, 'Buff'],
    }
...
def buff_index(db):
    db.execute(sql_buff)
    data = db.fetchall()
    return template('buff_index', data=data)

@route("/buff/delete", method="POST")
def buff_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_buff WHERE BuffID=%s;' % sid
...
def buff_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_buff%Y%m%d%H%M%S.xls'))
...
def buff_export(db):
    tmp_root = './tmp/'
    filename = current_time("buff_%Y%m%d%H%M.xls")  # 文件名
    error = ''
