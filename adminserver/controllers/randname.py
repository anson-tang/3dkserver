...
BACK_FILE = 'randname_back_up.xls'
table_name = 'tb_randname'

field_base = 'ID', 'FirstName', 'Male', 'Famale'
sql_base = 'SELECT %s FROM %s' % (','.join(field_base), table_name)
insert_sql = 'INSERT INTO %s (%s) VALUES (%s)' % (table_name, ','.join(field_base[1:]), ','.join(['%s'] * (len(field_base)-1)))

IMPORT_SQLS = {
        'RandName' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'RandName'],
    }

@route("/randname")
def randname_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('randname_index', data=data)

@route("/randname/delete", method="POST")
def randname_delete(db):
    rid = request.forms.rid.strip()
    sql = 'DELETE FROM %s WHERE ID=%s;' % (table_name, rid)
...
@route("/randname/import", method="POST")
def randname_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmprandname_%Y%m%d%H%M%S.xls'))
...
@route("/randname/export")
def randname_export(db):
    tmp_root = './tmp/'
    filename = current_time("randname_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/randname/recover")
def randname_recover(db):
    tmp_root = './tmp/'
