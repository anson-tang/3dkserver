...
BACK_FILE = 'achievement_back_up.xls'

field_achievement = 'ID', 'QuestType', 'Value', 'PreviousID', 'Description', 'RewardList', 'Icon', 'Quality', 'RewardDesc', 'IsEnd'
sql_achievement   = 'SELECT %s FROM tb_achievement' % ','.join(field_achievement)
insert_sql_skill   = 'INSERT INTO tb_achievement (%s) VALUES (%s)' % (','.join(field_achievement), ','.join(['%s'] * len(field_achievement)))

IMPORT_SQLS = {
        'Achievement'   : [insert_sql_skill, 'tb_achievement'],
    }

EXPORT_SQLS = {
        'tb_achievement' : [sql_achievement, field_achievement, 'Achievement'],
    }

@route("/achievement")
def achievement_index(db):
    db.execute(sql_achievement)
    data = db.fetchall()
    return template('achievement_index', data=data)

@route("/achievement/delete", method="POST")
def achievement_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_achievement WHERE ID=%s;' % sid
...
@route("/achievement/import", method="POST")
def achievement_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_achievement%Y%m%d%H%M%S.xls'))
...
@route("/achievement/export")
def achievement_export(db):
    tmp_root = './tmp/'
    filename = current_time("achievement_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/achievement/recover")
def achievement_recover(db):
    tmp_root = './tmp/'
