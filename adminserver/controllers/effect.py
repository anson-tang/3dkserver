...
BACK_FILE = 'effect_back_up.xls'
table_name = 'tb_effect'

field_base = 'ID', 'ActionType', 'ActionStartEffect', 'StartDelayTime', 'StartDuration', 'StartSound', 'ActionMovmentEffect', 'MovmentDelayTime', 'MovmentDuration', 'MoveMentSound', 'ActionTargetEffect', 'TargetDelayTime', 'TargetDuration', 'TargetSound'
...
IMPORT_SQLS = {
        'ActionEffect' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'ActionEffect'],
    }
...
def effect_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('effect_index', data=data)

@route("/effect/delete", method="POST")
def effect_delete(db):
    eid = request.forms.eid.strip()
    etype = request.forms.etype.strip()
    sql = 'DELETE FROM %s WHERE ID=%s AND ActionType=%s' % (table_name, eid, etype)
...
@route("/effect/import", method="POST")
def effect_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmpeffect_%Y%m%d%H%M%S.xls'))
...
@route("/effect/export")
def effect_export(db):
    tmp_root = './tmp/'
    filename = current_time("effect_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/effect/recover")
def effect_recover(db):
    tmp_root = './tmp/'
