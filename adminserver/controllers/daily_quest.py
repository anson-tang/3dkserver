...
BACK_FILE = 'daily_quest_back_up.xls'

field_daily_quest = 'ID', 'Title', 'Description', 'Icon', 'Requirements', 'Score', 'RewardList'
del_sql_daily_quest = 'DELETE FROM tb_daily_quest WHERE ID=%s;'

sql_daily_quest   = 'SELECT %s FROM tb_daily_quest' % ','.join(field_daily_quest)
insert_sql_daily_quest   = 'INSERT INTO tb_daily_quest (%s) VALUES (%s)' % (','.join(field_daily_quest), ','.join(['%s'] * len(field_daily_quest)))

excel_name = 'DailyQuest'
excel_export_filename = "DailyQuest_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_daily_quest, 'tb_daily_quest'],
    }

EXPORT_SQLS = {
        'tb_daily_quest' : [sql_daily_quest, field_daily_quest, excel_name],
    }

@route("/daily_quest")
def daily_quest_index(db):
    db.execute(sql_daily_quest)
    data = db.fetchall()
    return template('daily_quest_index', data=data)

@route("/daily_quest/delete", method="POST")
def daily_quest_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_daily_quest WHERE ID=%s;' % sid
    sql = del_sql_daily_quest % sid
...
@route("/daily_quest/import", method="POST")
def daily_quest_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_daily_quest%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/daily_quest")

@route("/daily_quest/export")
def daily_quest_export(db):
    tmp_root = './tmp/'
...
@route("/daily_quest/recover")
def daily_quest_recover(db):
    tmp_root = './tmp/'
