...
BACK_FILE = 'group_buy_back_up.xls'

field_group_buy = 'BuyType', 'ItemType', 'ItemID', 'ItemNum', 'OriginalPrice', 'CurrentPrice', 'LimitNum'
sql_group_buy   = 'SELECT %s FROM tb_group_buy' % ','.join(field_group_buy)
insert_sql_skill   = 'INSERT INTO tb_group_buy (%s) VALUES (%s)' % (','.join(field_group_buy), ','.join(['%s'] * len(field_group_buy)))

IMPORT_SQLS = {
        'Group_buy'   : [insert_sql_skill, 'tb_group_buy'],
    }

EXPORT_SQLS = {
        'tb_group_buy' : [sql_group_buy, field_group_buy, 'Group_buy'],
    }

@route("/group_buy")
def group_buy_index(db):
    db.execute(sql_group_buy)
    data = db.fetchall()
    return template('group_buy_index', data=data)

@route("/group_buy/delete", method="POST")
def group_buy_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_group_buy WHERE BuyType=%s;' % sid
...
@route("/group_buy/import", method="POST")
def group_buy_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_group_buy%Y%m%d%H%M%S.xls'))
...
@route("/group_buy/export")
def group_buy_export(db):
    tmp_root = './tmp/'
    filename = current_time("group_buy_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/group_buy/recover")
def group_buy_recover(db):
    tmp_root = './tmp/'
