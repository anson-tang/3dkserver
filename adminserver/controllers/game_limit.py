...
BACK_FILE = 'game_limit_back_up.xls'

field_game_limit = 'LimitID', 'LimitDesc', 'LimitValue'
del_sql_game_limit = 'DELETE FROM tb_game_limit WHERE LimitID=%s;'

sql_game_limit   = 'SELECT %s FROM tb_game_limit' % ','.join(field_game_limit)
insert_sql_game_limit   = 'INSERT INTO tb_game_limit (%s) VALUES (%s)' % (','.join(field_game_limit), ','.join(['%s'] * len(field_game_limit)))

excel_name = 'GameLimit'
excel_export_filename = "GameLimit_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_game_limit, 'tb_game_limit'],
    }

EXPORT_SQLS = {
        'tb_game_limit' : [sql_game_limit, field_game_limit, excel_name],
    }

@route("/game_limit")
def game_limit_index(db):
    db.execute(sql_game_limit)
    data = db.fetchall()
    return template('game_limit_index', data=data)

@route("/game_limit/delete", method="POST")
def game_limit_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_game_limit WHERE ID=%s;' % sid
    sql = del_sql_game_limit % sid
...
@route("/game_limit/import", method="POST")
def game_limit_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_game_limit%Y%m%d%H%M%S.xls'))
...
@route("/game_limit/export")
def game_limit_export(db):
    tmp_root = './tmp/'
...
@route("/game_limit/recover")
def game_limit_recover(db):
    tmp_root = './tmp/'
