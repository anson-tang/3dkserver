...
BACK_FILE = 'tomb_treasure_back_up.xls'

field_tomb_treasure = 'ID', 'ItemType', 'ItemID'
sql_tomb_treasure   = 'SELECT %s FROM tb_tomb_treasure' % ','.join(field_tomb_treasure)
insert_sql_skill   = 'INSERT INTO tb_tomb_treasure (%s) VALUES (%s)' % (','.join(field_tomb_treasure), ','.join(['%s'] * len(field_tomb_treasure)))

IMPORT_SQLS = {
        'Tomb_treasure'   : [insert_sql_skill, 'tb_tomb_treasure'],
    }

EXPORT_SQLS = {
        'tb_tomb_treasure' : [sql_tomb_treasure, field_tomb_treasure, 'Tomb_treasure'],
    }

@route("/tomb_treasure")
def tomb_treasure_index(db):
    db.execute(sql_tomb_treasure)
    data = db.fetchall()
    return template('tomb_treasure_index', data=data)

@route("/tomb_treasure/delete", method="POST")
def tomb_treasure_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_tomb_treasure WHERE ID=%s;' % sid
...
@route("/tomb_treasure/import", method="POST")
def tomb_treasure_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_tomb_treasure%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/tomb_treasure")

@route("/tomb_treasure/export")
def tomb_treasure_export(db):
    tmp_root = './tmp/'
    filename = current_time("tomb_treasure_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/tomb_treasure/recover")
def tomb_treasure_recover(db):
    tmp_root = './tmp/'
