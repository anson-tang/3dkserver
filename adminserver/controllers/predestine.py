...
BACK_FILE = 'predestine_back_up.xls'

field_predestine = 'ID', 'FellowID', 'Turn', 'PredestineName', 'PredestineDesc', 'ItemType', 'ItemID', 'AttributeList'
sql_predestine   = 'SELECT %s FROM tb_predestine' % ','.join(field_predestine)
insert_sql_skill   = 'INSERT INTO tb_predestine (%s) VALUES (%s)' % (','.join(field_predestine), ','.join(['%s'] * len(field_predestine)))

IMPORT_SQLS = {
        'Predestine'   : [insert_sql_skill, 'tb_predestine'],
    }

EXPORT_SQLS = {
        'tb_predestine' : [sql_predestine, field_predestine, 'Predestine'],
    }

@route("/predestine")
def predestine_index(db):
    db.execute(sql_predestine)
    data = db.fetchall()
    return template('predestine', data=data)

@route("/predestine/delete", method="POST")
def predestine_delete(db):
    sid = request.forms.sid.strip()
    sql = 'DELETE FROM tb_predestine WHERE ID=%s;' % sid
...
@route("/predestine/import", method="POST")
def predestine_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_predestine%Y%m%d%H%M%S.xls'))
...
@route("/predestine/export")
def predestine_export(db):
    tmp_root = './tmp/'
    filename = current_time("predestine_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/predestine/recover")
def predestine_recover(db):
    tmp_root = './tmp/'
