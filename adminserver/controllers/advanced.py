...
BACK_FILE = 'advanced_back_up.xls'
table_name = 'tb_fellow_advanced'

field_base = 'ID', 'FellowID', 'AdvancedLevel', 'FellowLevelLimit', 'Gold', 'ItemList', 'Life', 'Attack', 'PhysicsDefense', 'MagicDefense', 'BuffID', 'BuffName', 'BuffDescription'
...
IMPORT_SQLS = {
        'Advanced' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'Advanced'],
    }

@route("/advanced")
def advanced_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('advanced_index', data=data)

@route("/advanced/delete", method="POST")
def advanced_delete(db):
    aid = request.forms.aid.strip()
    sql = 'DELETE FROM %s WHERE ID=%s;' % (table_name, aid)
...
@route("/advanced/import", method="POST")
def advanced_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmpadvanced_%Y%m%d%H%M%S.xls'))
...
@route("/advanced/export")
def advanced_export(db):
    tmp_root = './tmp/'
    filename = current_time("advanced_%Y%m%d%H%M.xls")  # 文件名
    error = ''
...
@route("/advanced/recover")
def advanced_recover(db):
    tmp_root = './tmp/'
