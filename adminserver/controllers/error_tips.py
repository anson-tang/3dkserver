...
BACK_FILE = 'error_tips_back_up.xls'

field_error_tips = 'ErrorID', 'ErrorKey', 'ErrorType', 'ErrorLink'
del_sql_error_tips = 'DELETE FROM tb_error_tips WHERE ErrorID=%s;'

sql_error_tips   = 'SELECT %s FROM tb_error_tips' % ','.join(field_error_tips)
insert_sql_error_tips   = 'INSERT INTO tb_error_tips (%s) VALUES (%s)' % (','.join(field_error_tips), ','.join(['%s'] * len(field_error_tips)))

excel_name = 'ErrorTips'
excel_export_filename = "ErrorTips_%Y%m%d%H%M.xls"

IMPORT_SQLS = {
        excel_name   : [insert_sql_error_tips, 'tb_error_tips'],
    }

EXPORT_SQLS = {
        'tb_error_tips' : [sql_error_tips, field_error_tips, excel_name],
    }

@route("/error_tips")
def error_tips_index(db):
    db.execute(sql_error_tips)
    data = db.fetchall()
    return template('error_tips_index', data=data)

@route("/error_tips/delete", method="POST")
def error_tips_delete(db):
    sid = request.forms.sid.strip()
    #sql = 'DELETE FROM tb_error_tips WHERE ID=%s;' % sid
    sql = del_sql_error_tips % sid
...
@route("/error_tips/import", method="POST")
def error_tips_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmp_error_tips%Y%m%d%H%M%S.xls'))
...
@route("/error_tips/export")
def error_tips_export(db):
    tmp_root = './tmp/'
...
@route("/error_tips/recover")
def error_tips_recover(db):
    tmp_root = './tmp/'
