...
BACK_FILE = 'limit_fellow_pool_back_up.xls'
table_name = 'tb_limit_fellow_pool'

field_base = 'ID', 'Level', 'Rate', 'ItemType', 'ItemId', 'ItemNum', 'Notice'
...
IMPORT_SQLS = {
        'LimitFellowPool' : [insert_sql, table_name],  # 第二个元素为数据表名
    }

EXPORT_SQLS = {
        table_name : [sql_base, field_base, 'LimitFellowPool'],
    }

@route("/limit_fellow_pool")
def limit_fellow_pool_index(db):
    db.execute(sql_base)
    data = db.fetchall()
    return template('limit_fellow_pool_index', data=data)

@route("/limit_fellow_pool/delete", method="POST")
def limit_fellow_pool_delete(db):
    cid = request.forms.cid.strip()
    sql = 'DELETE FROM %s WHERE ID=%s;' % (table_name, cid)
...
@route("/limit_fellow_pool/import", method="POST")
def limit_fellow_pool_import(db):
    ...
    if data and data.file:
        ...
        if not isdir(tmp_root):  # 若目录tmp_root不存在，则创建
            os.mkdir(tmp_root)
        tmp_filename = os.path.join(tmp_root, current_time('tmplimit_fellow_pool_%Y%m%d%H%M%S.xls'))
    ...
    if error:
        ...
        redirect("/limit_fellow_pool")

@route("/limit_fellow_pool/export")
...
@route("/limit_fellow_pool/recover")
def limit_fellow_pool_recover(db):
    tmp_root = './tmp/'
