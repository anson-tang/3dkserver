...
function treasureshard_rate_delete(sid) {
    if (confirm("确定要删除TreasureshardID [ " + sid + " ]吗?")) {
        $.post('/treasureshard_rate/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/treasureshard_rate');
            }
        });
    }
