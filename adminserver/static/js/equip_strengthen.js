...
function equip_strengthen_delete(sid) {
    if (confirm("确定要删除StrengthenLevel [ " + sid + " ]吗?")) {
        $.post('/equip_strengthen/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/equip_strengthen');
            }
        });
    }
