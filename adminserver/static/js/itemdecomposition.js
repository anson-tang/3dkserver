...
function itemdecomposition_delete(itype, quality) {
    if (confirm("确定要删除[ " + itype + " " + quality + " ]吗?")) {
        $.post('/itemdecomposition/delete', {itype:itype, quality:quality}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/itemdecomposition');
            }
        });
    }
