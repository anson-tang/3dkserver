...
function group_buy_reward_list_delete(sid) {
    if (confirm("确定要删除group_buy_reward_list[ " + sid + " ]吗?")) {
        $.post('/group_buy_reward_list/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/group_buy_reward_list');
            }
        });
    }
