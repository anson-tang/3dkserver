...
function elitescene_delete(sid) {
    if (confirm("确定要删除副本[ " + sid + " ]吗?")) {
        $.post('/elitescene/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/elitescene');
            }
        });
    }
...
function elitescene_monster_del(sid, tid) {
    if (confirm("确定删除该条Dungeon吗？")) {
        $.post("/elitescene/delmonster", {sid:sid, tid:tid}, function(result){
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID：" + result.result );
            } else {
                elitescene_monsters_get(sid);
            }   
        }); 
    } else {
...
function elitescene_monsters_get(sid) {
    $.post('/elitescene/monster/get', {sid: sid}, function(result) {
        ...
        if (!result.result) {
            ...
            $(result.data).each(function() {
                data_table.fnAddData([
                    ...
                    $(this)[2],
                    $(this)[3],
                    '</button><button id="ldel" style="width:20px;height:20px;" onclick="return elitescene_monster_del(' + sid + ', ' + $(this)[0] + ');"></button>'
                  ]);
