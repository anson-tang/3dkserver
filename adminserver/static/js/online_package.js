...
function online_package_delete(sid, pid) {
    if (confirm("确定要删除PackageGroup [ " + sid + " ], PackageID [" + pid + "] 吗?")) {
        $.post('/online_package/delete', {sid: sid, pid: pid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/online_package');
            }
        });
    }
