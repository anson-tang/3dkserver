...
function open_server_activity_delete(sid) {
    if (confirm("确定要删除open_server_activity[ " + sid + " ]吗?")) {
        $.post('/open_server_activity/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/open_server_activity');
            }
        });
    }
