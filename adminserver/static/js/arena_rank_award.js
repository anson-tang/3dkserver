...
var c_lang = 0
function arena_rank_award_delete(sid) {
    if (confirm("确定要删除Rank [ " + sid + " ]吗?")) {
        $.post('/arena_rank_award/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/arena_rank_award');
            }
        });
    }
...
$(function() {
    ...
    oTable = $("#data").dataTable({
        ...
        "bProcessing":true,
        "bServerSide":true,
        "sAjaxSource":"/arena_rank_award/page",
        "sDom": '<"toolbar">frtip', 
        "scrollY":400, 
        "scrollX":true
    });
