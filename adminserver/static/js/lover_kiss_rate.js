...
function lover_kiss_rate_delete(sid) {
    if (confirm("确定要删除lover_kiss_rate[ " + sid + " ]吗?")) {
        $.post('/lover_kiss_rate/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/lover_kiss_rate');
            }
        });
    }
