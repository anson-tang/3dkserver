...
function treasure_predestine_delete(sid) {
    if (confirm("确定要删除TreasureID [ " + sid + " ]吗?")) {
        $.post('/treasure_predestine/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/treasure_predestine');
            }
        });
    }
