...
function activity_lottery_delete(sid) {
    if (confirm("确定要删除 ID [ " + sid + " ]吗?")) {
        $.post('/activity_lottery/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/activity_lottery');
            }
        });
    }
...
$(function() {
    ...
    oTable = $("#data").dataTable({
        ...
        "bProcessing":true,
        "bServerSide":true,
        "sAjaxSource":"/activity_lottery/page",
        "sDom": '<"toolbar">frtip', 
        "scrollY":400, 
        "scrollX":true
        });
