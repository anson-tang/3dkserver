...
function scene_sweep_cost_delete(sid) {
    if (confirm("确定要删除ResetTime [ " + sid + " ]吗?")) {
        $.post('/scene_sweep_cost/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/scene_sweep_cost');
            }
        });
    }
