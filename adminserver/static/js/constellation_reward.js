...
function constellation_reward_delete(turn) {
    if (confirm("确定要删除 Turn [ " + turn + " ]吗?")) {
        $.post('/constellation_reward/delete', {turn: turn}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/constellation_reward');
            }
        });
    }
