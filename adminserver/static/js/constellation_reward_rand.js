...
function constellation_reward_rand_delete(cid) {
    if (confirm("确定要删除 ID [ " + cid + " ]吗?")) {
        $.post('/constellation_reward_rand/delete', {cid: cid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/constellation_reward_rand');
            }
        });
    }
