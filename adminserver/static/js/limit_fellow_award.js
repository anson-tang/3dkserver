...
function limit_fellow_award_delete(sid) {
    if (confirm("确定要删除AttributeID [ " + sid + " ]吗?")) {
        $.post('/limit_fellow_award/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/limit_fellow_award');
            }
        });
    }
