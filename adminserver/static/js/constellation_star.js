...
function constellation_star_delete(sid) {
    if (confirm("确定要删除Turn [ " + sid + " ]吗?")) {
        $.post('/constellation_star/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/constellation_star');
            }
        });
    }
