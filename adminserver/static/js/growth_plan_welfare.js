...
function growth_plan_welfare_delete(sid) {
    if (confirm("确定要删除Level [ " + sid + " ]吗?")) {
        $.post('/growth_plan_welfare/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/growth_plan_welfare');
            }
        });
    }
