...
function lover_kiss_reward_preview_delete(sid) {
    if (confirm("确定要删除lover_kiss_reward_preview[ " + sid + " ]吗?")) {
        $.post('/lover_kiss_reward_preview/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/lover_kiss_reward_preview');
            }
        });
    }
