...
function limit_fellow_level_delete(level) {
    if (confirm("确定要删除[ " + cid + " ]吗?")) {
        $.post('/limit_fellow_level/delete', {level: level}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/limit_fellow_level');
            }
        });
    }
