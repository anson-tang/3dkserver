...
var c_lang = 0
function arena_robot_delete(sid) {
    if (confirm("确定要删除ID [ " + sid + " ]吗?")) {
        $.post('/arena_robot/delete', {sid: sid}, function(result) {
            if (result.result) {
                alert("删除失败！原因：" + result.data + ", ID: " + result.result);    
            } else {
                $(location).attr('href', '/arena_robot');
            }
        });
    }
